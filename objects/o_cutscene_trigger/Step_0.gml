...
if(currentState == cutSceneStates.Active) 
{
	switch (currentStep) {
			...
			target = o_thicc;
			var speak_zoneX = target.x - (sprite_get_width(box));
			var speak_zoneY = target.y - sprite_get_height(target)*1.5;
			...
			if(!instance_exists(o_dialogue_box)) //if there is no dialogue box, create a dialogue box
				dB = instance_create_layer(speak_zoneX,speak_zoneY,"Instances", o_dialogue_box); //create the dialogue box and pick its position
		...
		if(!instance_exists(o_dialogue_box))
			dB = instance_create_layer(speak_zoneX,speak_zoneY,"Instances", o_dialogue_box);
		...
		target = o_thicc;
		var speak_zoneX = target.x - (sprite_get_width(target));
		var speak_zoneY = target.y - sprite_get_height(target)*1.5;
		if(!instance_exists(o_dialogue_box))
			dB = instance_create_layer(speak_zoneX,speak_zoneY,"Instances", o_dialogue_box);
